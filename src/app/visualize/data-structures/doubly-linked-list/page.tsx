...
const Arrow = ({ direction }: { direction: "forward" | "backward" }) => (
  <motion.div
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
    exit={{ opacity: 0 }}
    className={`flex-shrink-0 ${direction === "backward" ? "rotate-180" : ""}`}
  >
          ...
          <AnimatePresence>
            {nodes.map((node, index) => (
              <React.Fragment key={node.id}>
                {index > 0 && <Arrow direction="backward" />}
                <Node
                  node={node}
                  index={index}
                  isHighlighted={highlightIndex === index}
                />
                {index < nodes.length - 1 && <Arrow direction="forward" />}
              </React.Fragment>
            ))}
            {nodes.length === 0 && (
