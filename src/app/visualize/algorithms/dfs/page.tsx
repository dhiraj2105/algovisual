...
export default function InteractiveDFSVisualizer() {
  ...
  const startEdge = (fromId: string) => {
    if (edgeMode.from === fromId) setEdgeMode({ from: null });
    else setEdgeMode({ from: fromId });
  };
  ...
  // DFS Traversal (recursive with stack simulation)
  const dfs = async (start: string) => {
    setIsRunning(true);
    const vis: Set<string> = new Set();
    const stk: string[] = [start];
    setStack([...stk]);
    ...
    while (stk.length > 0) {
      ...
      const neighbors = edges
        ...
        .map((e) => e.to)
        .filter((n) => !vis.has(n));
      for (let i = neighbors.length - 1; i >= 0; i--) stk.push(neighbors[i]);
      setStack([...stk]);
      await sleep(400);
    }

    setCurrentNode(null);
    setStepExplanation("✅ DFS traversal completed");
    setIsRunning(false);
  };
    ...
    <div className="flex flex-col h-screen w-full bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
        ...
        <AnimatePresence>
          {stack.map((sItem, idx) => (
            <motion.div
              ...
              className="px-3 py-1 bg-yellow-200 dark:bg-yellow-700 rounded shadow text-lg font-semibold"
            >
              {sItem}
            </motion.div>
          ))}
      ...
      <div className="h-1/5 p-4 bg-white dark:bg-gray-800 rounded-t-2xl shadow flex flex-col gap-3">
        ...
        <div className="text-xs text-gray-500">
          Click nodes to add edges. DFS starts from first node or user-selected.
        </div>
      </div>
    </div>
  );
